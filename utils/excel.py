...
from modules.cherrycommon.dictutils import flatten_value
...
def write_cell_value(value):
    ...
    if isinstance(value, (float, int, str, type(None), Formula)):
        return value
    else:
            ...
            for r in range(0, s.nrows):
                ...
                if sheet_row:
                    ...
                    if pattern:
                        ...
                        style = CELL
                    sheet.add_row([read_cell_value(cell.value) for cell in sheet_row], style)
        ...
        for row_num, row in enumerate(self.rows):
            ...
            for cell_num, value in enumerate(row.cells):
                ...
                if not isinstance(value, Formula):
                    col_widths[cell_num] = min(max(col_widths.get(cell_num, 10), len(str(value))), 54)

        for num, width in col_widths.items():
