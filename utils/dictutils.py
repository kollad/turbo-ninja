...
def dump_value(value):
    ...
    if isinstance(value, str):
        return str(value)
    elif isinstance(value, (tuple, list, set, GeneratorType)):
        return list(map(dump_value, value))
    elif isinstance(value, dict):
        # TODO: Raise an exception if key is empty string or None.
        return dict((key, dump_value(value)) for key, value in value.items()
...
def flatten_value(value):
    if isinstance(value, (dict, DictView)):
        return dict((k, flatten_value(v)) for k, v in value.items())
    elif isinstance(value, (list, ListView)):
        return list(map(flatten_value, value))
...
def merge(target, source, keep_none=False, skip_empty=True, skip_fields=()):
    if source is None:
        return None
    for key, source_value in list(source.items()):
...
def get_value(document, field, default=_default, flatten=True):
    ...
    :param: flatten: If set to None, value will be returned as is. If set to False - value will be dumped.
                     If True - value will be flattened.
...
def get_schema(documents, skip_nested=False, keep_none=False, nested_level=None):
    ...
    schema = []
    queue = sorted([(field, value, 0) for field, value in documents_union.items()])
    while queue:
...
class ListView(Sequence):
    ...
    def dump(self):
        return list(map(dump_value, self))
...
class MappingView(Mapping):
    ...
    def dump(self):
        return dict((key, dump_value(value)) for key, value in self.items())
...
class Diffed(MappingView, MutableMapping):
    ...
    def get_current_diff(self):
        ...
        """
        return next(self.lookup())

    def _get_diffed_item(self, item, value, diffs):
        diffs.reverse()
        last_diff = self._diffs[-1]
        if item not in last_diff:
            last_value = {}
    ...
    def __getitem__(self, item):
        ...
        diffs = []
        for diff in reversed(self._diffs):
            if item in diff:
                diff_value = diff[item]
                if isinstance(diff_value, dict):
                    diffs.append(diff_value)
                    continue
                elif isinstance(diff_value, DictView):
                    diffs.append(diff_value)
                    return self._get_diffed_item(item, {}, diffs)
                elif diffs:
                    return self._get_diffed_item(item, {}, diffs)
                elif diff_value is None:
                    raise KeyError('{} not found'.format(item))
                else:
                    return view_value(diff_value)

        value = self._data.get(item)
        if diffs:
            if not isinstance(value, (dict, DictView)):
                return self._get_diffed_item(item, {}, diffs)
            else:
                return self._get_diffed_item(item, value, diffs)
        elif isinstance(value, (dict, DictView)):
            return self._get_diffed_item(item, value, [])
        elif value is None:
            raise KeyError('{} not found'.format(item))
        else:
            return view_value(value)
    ...
    def __contains__(self, item):
        for container in self.lookup():
            if item in container:
                return container[item] is not None
        return False
    ...
    def add_diff(self, *diffs):
        for diff in diffs:
            if not isinstance(diff, (dict, DictView, type(None))):
                raise TypeError(
    ...
    def __setitem__(self, item, value):
        ...
        if diff is None:
            diff = {}
            self._diffs[-1] = diff
        if isinstance(value, (type(None), int, float, str, list, set, tuple, dict, DictView)):
            diff[item] = value
        else:
...
def check_data_format(data_format=JSON):
    ...
    if data_format == JSON:
        ...
        _module = _supported_data_formats[XJSON] = utils._xjson
    elif data_format == YAML:
        try:
            import modules.cherrycommon._yaml
        except ImportError:
            _supported_data_formats[YAML] = False
            raise
        else:
            _module = _supported_data_formats[YAML] = modules.cherrycommon._yaml
    elif data_format == MSGPACK:
        try:
            import msgpack
        except ImportError:
            _supported_data_formats[MSGPACK] = False
            raise
        else:
            _module = _supported_data_formats[MSGPACK] = msgpack
    else:
